...
export default function CampRegistration() {
  ...
  useEffect(() => {
    ...
    }, searchTerm ? 300 : 0); // Debounce search, but not session changes

    return () => clearTimeout(timer);
  }, [selectedSession, searchTerm]); // Remove fetchRegistrations from deps
  ...
  const initializeSessions = async () => {
    try {
      ...
      const sessionsWithData = data.sessions.filter(s => s.hasData && s.registrationCount > 0);
      setAvailableSessions(sessionsWithData);
      
      // Set default to the most recent session with data
      if (sessionsWithData.length > 0) {
        const mostRecent = sessionsWithData.sort((a, b) => new Date(b.date) - new Date(a.date))[0];
  ...
  const fetchRegistrations = async () => {
    ...
    try {
      ...
      const registrationsArray = Array.isArray(registrationData) ? registrationData : [];
  ...
  const generateTableColumns = (data) => {
    ...
    const columnConfigs = [
      { key: 'childName', label: 'Child Name', priority: 1, type: 'text', width: '150px' },
      { key: 'childAge', label: 'Age', priority: 2, type: 'text', width: '60px' },
      { key: 'firstName', label: 'Parent First', priority: 3, type: 'text', width: '120px' },
      { key: 'lastName', label: 'Parent Last', priority: 4, type: 'text', width: '120px' },
      { key: 'email', label: 'Email', priority: 5, type: 'email', width: '200px' },
      { key: 'phone', label: 'Phone', priority: 6, type: 'phone', width: '120px' },
      { key: 'address1', label: 'Address', priority: 7, type: 'text', width: '200px' },
      { key: 'city', label: 'City', priority: 8, type: 'text', width: '120px' },
      { key: 'state', label: 'State', priority: 9, type: 'text', width: '80px' },
      { key: 'zip', label: 'Zip', priority: 10, type: 'text', width: '80px' },
      { key: 'childAlergies', label: 'Allergies', priority: 11, type: 'text', width: '150px' },
      { key: 'permission', label: 'Permission', priority: 12, type: 'boolean', width: '90px' },
      { key: 'marketingConsent', label: 'Marketing', priority: 13, type: 'boolean', width: '90px' },
      { key: 'worship', label: 'Worship', priority: 14, type: 'boolean', width: '80px' },
      { key: 'bibleStories', label: 'Bible Stories', priority: 15, type: 'boolean', width: '100px' },
      { key: 'lunch', label: 'Lunch', priority: 16, type: 'boolean', width: '70px' },
      { key: 'artCamp', label: 'Art Camp', priority: 17, type: 'boolean', width: '80px' },
    ];

    // Filter columns that actually exist in the data and add special columns
    const availableColumns = columnConfigs
      .filter(col => regData.hasOwnProperty(col.key) && regData[col.key] !== undefined)
      .sort((a, b) => a.priority - b.priority);

    // Add special computed columns
    const specialColumns = [
      { key: 'schedule', label: 'Schedule', type: 'schedule', width: '150px', priority: 20 },
      { key: 'registrationDate', label: 'Registered', type: 'date', width: '100px', priority: 21 },
      { key: 'actions', label: 'Actions', type: 'actions', width: '100px', priority: 22 }
    ];
    ...
    switch (column.type) {
      case 'email':
        return data[column.key] ? (
          <a href={`mailto:${data[column.key]}`} style={{ color: "#007bff", textDecoration: "none" }}>
            {data[column.key]}
          </a>
        ) : '';
      ...
      case 'phone':
        return data[column.key] ? (
          <a href={`tel:${data[column.key]}`} style={{ color: "#007bff", textDecoration: "none" }}>
            {data[column.key]}
          </a>
        ) : '';
    ...
    <div className="managemembers-container">
      ...
      >
        ...
        >
          ...
          >
            {availableSessions.map((session) => (
              <option key={session.value} value={session.value}>
                {session.label}{" "}
                {session.hasData ? `(${session.registrationCount})` : "(Empty)"}
              </option>
            ))}
          </select>
          ...
          {sessionStats && (
            ...
            >
              {sessionStats.count} Registrations
            </span>
          )}
        </div>
      ...
      >
        <h3 style={{ margin: "0 0 5px 0", color: "#004085" }}>
          📅 Viewing: {getCurrentSessionDisplay()}
        </h3>
          ...
          <tbody>
            {registrations.length === 0 ? (
              ...
              registrations.map((reg) => {
                // Handle both old format (formData wrapper) and new format (direct fields)
                const data = reg.formData || reg;

                // Build schedule summary
                const scheduleItems = [];
                if (data.worship) scheduleItems.push("Worship");
                if (data.bibleStories) scheduleItems.push("Bible Stories");
                if (data.lunch) scheduleItems.push("Lunch");
                if (data.artCamp) scheduleItems.push("Art Camp");
                const scheduleText =
                ...
                return (
                  <tr key={reg._id} className="managemembers-table-tbody-row">
                    <td style={{ fontWeight: "600" }}>{data.childName}</td>
                    <td>{data.childAge}</td>
                    <td>
                    ...
                    <td style={{ fontSize: "0.9rem", color: "#6c757d" }}>
      ...
      {modalVisible && (
        <CampModal
          ...
          sessionDisplay={getCurrentSessionDisplay()}
          onClose={closeModal}
